...
export function useDelivery() {
  const loadDelivery = async (formData, cart, cartTotal) => {
    try {
      ...
      if (!token) {
        throw new Error(
          "Usuário não está logado. Faça login para finalizar o pedido.",
        );
      }
      ...
      alert(
        "Pedido registrado com sucesso! O pagamento será gerado quando o separador confirmar o pedido.",
      );
  ...
  const loadShipping = async (formData) => {
    try {
      ...
      if (response.customer.acepted == false) {
        ...
        throw new Error(
          "Erro ao gerar pagamento: link de pagamento não encontrado",
        );
      }
